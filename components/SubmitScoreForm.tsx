...
export const SubmitScoreForm: React.FC<SubmitScoreFormProps> = ({ onSubmited, onCancel }) => {
  ...
  if (success) {
    return (
      <section className="relative isolate mx-auto max-w-5xl overflow-hidden rounded-[2rem] bg-slate-950 text-white shadow-2xl shadow-indigo-200/70 sm:rounded-[2.5rem]">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_12%_10%,rgba(99,102,241,.48),transparent_34%),radial-gradient(circle_at_88%_86%,rgba(217,70,239,.35),transparent_35%)]" /><div className="absolute inset-0 opacity-[0.08] [background-image:linear-gradient(rgba(255,255,255,.8)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.8)_1px,transparent_1px)] [background-size:28px_28px]" />
        <div className="relative grid gap-8 px-6 py-10 sm:px-10 sm:py-14 lg:grid-cols-[1.08fr_.92fr] lg:gap-12">
          <div className="flex flex-col justify-center"><span className="inline-flex w-fit items-center gap-2 rounded-full border border-emerald-300/20 bg-emerald-300/10 px-3 py-1.5 text-xs font-bold text-emerald-200"><CheckCircle2 className="h-3.5 w-3.5" />資料回報完成</span><div className="mt-7 flex h-20 w-20 items-center justify-center rounded-[1.7rem] bg-white text-emerald-600 shadow-xl shadow-black/20"><CheckCircle2 className="h-10 w-10" /></div><h2 className="mt-7 text-4xl font-black leading-[1.1] tracking-tight sm:text-5xl">送出成功！<br /><span className="bg-gradient-to-r from-indigo-200 via-white to-fuchsia-200 bg-clip-text text-transparent">謝謝你的分享</span></h2><p className="mt-5 max-w-lg font-medium leading-7 text-slate-300">你的回報會成為下一位考生的參考，讓全國落點分析持續更貼近真實情況。</p><button onClick={onSubmited} className="mt-8 inline-flex w-fit items-center gap-2 rounded-2xl border border-white/15 bg-white/10 px-5 py-3.5 text-sm font-bold text-white transition hover:bg-white hover:text-slate-900">返回序位資料首頁 <ChevronRight className="h-4 w-4" /></button></div>
          <div className="rounded-[1.75rem] border border-white/15 bg-white/10 p-5 backdrop-blur-xl sm:p-7"><div className="flex items-start gap-4"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-amber-300/15 text-amber-200"><Gift className="h-6 w-6" /></span><div><p className="text-sm font-black text-white">完成回報禮</p><p className="mt-1 text-sm font-medium leading-6 text-slate-300">你已獲得「全國落點分析」專屬邀請碼。</p></div></div><a href={`https://tyctw.github.io/spare/?invite=${generateInvitationCode()}`} target="_blank" rel="noopener noreferrer" className="group mt-8 flex items-center justify-between rounded-2xl bg-white px-5 py-4 font-bold text-slate-900 shadow-lg shadow-black/20 transition hover:-translate-y-0.5 hover:bg-indigo-50"><span>前往全國落點分析</span><span className="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-600 text-white transition-transform group-hover:translate-x-1"><ExternalLink className="h-4 w-4" /></span></a><a href="https://tyctw.github.io/invite/" target="_blank" rel="noopener noreferrer" className="mt-3 flex items-center justify-center gap-2 rounded-2xl border border-white/15 bg-white/5 px-5 py-3.5 text-sm font-bold text-indigo-100 transition hover:bg-white/10"><Key className="h-4 w-4" />已有邀請碼？前往使用</a><div className="mt-5 flex items-start gap-2 rounded-xl border border-amber-200/10 bg-amber-200/10 px-3.5 py-3 text-xs font-medium leading-5 text-amber-100"><Clock className="mt-0.5 h-4 w-4 shrink-0" />邀請碼將在本小時結束後失效，請儘早使用。</div></div>
        </div>
      </section>
    );
  }
